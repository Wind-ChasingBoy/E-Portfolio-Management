...
from math import gcd  # Greatest Common Denominator
...
def Generate_certificate(data, QR):
    ...
    draw.text(xy=(430, 1300), text='{}'.format(
        "Awarded by the College of Computer Science "), fill=(0, 0, 0), font=font)
    draw.text(xy=(550, 1360), text='{}'.format(
        "and Electronic Engineering, Hunan University"), fill=(0, 0, 0), font=font)
        ...
        if len(QR) != 0:
            ...
            try:
                if len(eval(QR_data)) == 3 and len(eval(QR_data)[0]) == 8:
                    ...
                    pfhash = contract2.functions.getCertificateInfo(
                        str(eval(QR_data)[0][0])).call()[2].hex()
...
app.config['MYSQL_DB'] = 'e_portfolio'
...
def getCertificate():
    try:
        if contract2.functions.getCertificateInfo(request.form['cert_no']).call()[0]:
            result = 1
            path = os.path.join(
                app.config['UPLOAD_FOLDER'], request.form['cert_no']+".png")
        else:
            result = 2
            path = ''
    except Exception:
...
quorum_url = "http://127.0.0.1:8545"
...
def setPortfolio():
    pf_number = "P202302-"+str(countPortfolioEntry()+1)
    data = Struct1(pf_number, request.form['pf_name'], request.form['reg_dtime'], request.form['reg_name'],
                   request.form['professor_name'], public_key, int(request.form['score']), 'Evaluated', 'www.google.com')
    # # update the greeting
    tx_hash = contract1.functions.newPortfolio(data.pf_no, data.pf_name, data.reg_dtime, data.reg_name,
                                               data.professor_name, data.professor_pk, data.score, data.pf_status, data.pf_url).transact()

    # # Wait for transaction to be mined...
    web3.eth.waitForTransactionReceipt(tx_hash)

    cert_number = "C202302-"+str(countCertificateEntry()+1)
    organizationID = "Hunan University"
    cdata = [cert_number, pf_number, request.form['pf_name'], organizationID, request.form['reg_name'],
             request.form['professor_name'], str(request.form['score']), request.form['reg_dtime']]
    pfhash = bytes.fromhex(hash_sha256(str(cdata)))
    signature = bytes.fromhex(sign(private_key, pfhash))
    data2 = Struct2(cert_number, pf_number, organizationID,
                    pfhash, signature, public_key)
    # # update the greeting
    tx_hash2 = contract2.functions.issuePortfolioCertificate(
        data2.cert_no, data2.pf_no, data2.organizationID, data2.pf_hash, data2.cert_signature, data2.professor_pk).transact()
...
def printPortfolio():
    # # Display the new greeting value
    try:
        data = contract1.functions.getPortfolioInfo(
            request.form['pf_no']).call()
        if data[0]:
            result = {
                'pf_no': data[0],
                'pf_name': data[1],
